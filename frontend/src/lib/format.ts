 ...
 *
 * Es lo que se manda al backend como `created_at`. Con `toISOString()` se
 * enviaría UTC, y como el backend guarda en columnas DATETIME sin zona y compara
 * contra su propio `datetime.now()`, una venta de las 19:00 en Costa Rica
 * quedaría registrada a la 01:00 del día siguiente: el arqueo del turno de noche
 * se partiría en dos y los reportes por día saldrían corridos.
 *
 * Todo el sistema —contenedor incluido, vía `TZ`— vive en la zona del negocio.
 */
