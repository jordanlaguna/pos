<script lang="ts">
	...
	import { issuerLines, returnedTotal, type DocumentProps } from '$lib/documents';
...
<svelte:head>
	{@html `<style>@media print { @page { size: ${doc.ancho_tiquete}mm auto; margin: 3mm; } }</style>`}
</svelte:head>
...
>
	...
	<footer class="border-t border-dashed border-[var(--border)] pt-4 text-center">
		{#if doc.mensaje_gracias}
			<p class="text-xs text-[var(--text-muted)]">{doc.mensaje_gracias}</p>
		{/if}
		{#if doc.leyenda}
			<p class="mt-1 text-[10px] text-[var(--text-subtle)]">{doc.leyenda}</p>
		{/if}
	</footer>
</article>
