...
>
	...
	>
		<div class="min-w-0">
			<h1 class="truncate text-2xl font-bold tracking-tight uppercase">
				{settings.negocio.nombre}
			</h1>
			{#if settings.negocio.razon_social && settings.negocio.razon_social !== settings.negocio.nombre}
				<p class="truncate text-sm opacity-80">{settings.negocio.razon_social}</p>
			{/if}
		</div>
	...
	<div class="px-8 py-6">
		...
		<div class="mb-7 grid gap-6 sm:grid-cols-2">
			<section>
				...
				<p class="text-sm font-bold">{settings.negocio.nombre}</p>
		...
		<div class="mt-8 grid gap-6 text-xs sm:grid-cols-2">
			...
			{#if doc.notas}
				<section>
					...
					<p class="leading-relaxed whitespace-pre-line text-slate-600">{doc.notas}</p>
				</section>
			{/if}
		</div>
	</div>
	...
	>
		{#if doc.mensaje_gracias}<p class="font-semibold">{doc.mensaje_gracias}</p>{/if}
		{#if doc.leyenda}<p class="mt-0.5 opacity-80">{doc.leyenda}</p>{/if}
	</footer>
</article>
