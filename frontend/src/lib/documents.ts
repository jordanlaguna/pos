 ...
 * se ve. La que decide es `settings.documento.plantilla`.
 */
...
export function documentTitle(settings: Settings): string {
	return settings.electronica.activa ? 'Factura electrónica' : 'Factura';
}

/** Datos del emisor listos para imprimir, sin las líneas vacías. */
export function issuerLines(settings: Settings): string[] {
	const { negocio } = settings;
	return [
		negocio.razon_social && negocio.razon_social !== negocio.nombre ? negocio.razon_social : '',
		negocio.identificacion ? `Cédula ${negocio.identificacion}` : '',
		negocio.direccion,
